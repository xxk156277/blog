...
- source-map：外部，错误代码准确信息 和 源代码的错 误位置
...
oneOf：匹配到 loader 后就不再向后进行匹配，优化生产环境的`打包构建速度`
...
dll(动态连接库)：让某些库单独打包，后直接引入到 build 中。可以在 code split 分割出 node_modules 后再用 dll 更细的分割，优化代码运行的性能。
...
`HardSourceWebpackPlugin`是 webpack 的插件，为模块提供中间缓存步骤。效果：第一次构建将花费正常的时间。第二个构建将显着更快。
...
```js
    new HardSourceWebpackPlugin({
        ...
        cachePrune: {
            ...
            sizeThreshold: 5000 * 1024 * 102
        }
    })
```

**cacheDirectory**：写入缓存的地址

**info：**控制来自`hard-source`的消息量。

- level:要报告的日志消息的级别
- mode

**environmentHash**：决定缓存（cache文件）是否需要更新

**cachePrune：**

`hard-source`默认情况下，当 webpack 配置更改时会创建缓存。每个缓存都保存所有数据的副本以创建构建，因此它们可以变得非常大。一旦缓存被认为“足够旧”而不太可能被重用，`hard-source`将删除它以自动释放空间。

- maxAge：超过`maxAge`毫秒的缓存会被考虑自动删除。
- sizeThreshold：要删除缓存，所有缓存的总和必须超过此阈值。
...
1. url：<String> 默认[http://localhost](https://link.segmentfault.com/?url=http%3A%2F%2Flocalhost):8080。
